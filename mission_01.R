...
nrow(rev) # rev 변수에 몇 건의 데이터가 있는지 확인해 봅니다
wordcount <- table(rev)
kk=head(sort(wordcount, decreasing=T),30)
tt=barplot(kk,col=rainbow(30),ylim=c(0,30),las=1)
text(tt,kk,label=paste0(kk,"건"),pos=3,cex=1.2)


#Step 9. Word Cloud 형태의 그래픽으로 출력합니다

library(RColorBrewer) # 화면에 출력할 컬러를 사용할 라이브러리를 Loading 합니다.
palete <- brewer.pal(9,"Set1") # 글자 색을 지정합니다.
